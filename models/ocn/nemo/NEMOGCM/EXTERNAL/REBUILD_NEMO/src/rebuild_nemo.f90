PROGRAM rebuild_nemo
   ...
   !!
   !!  P.G. Fogli, CMCC, Nov 2012 - Read namelist from standard input if
   !!
   !!-------------------------------------------------------------------------
   !!
   !!  The code reads the filestem and number of subdomains from the namelist file nam_rebuild
   !!  or from standard input if nam_rebuild does not exist (P.G. Fogli, CMCC, Nov 2012).
   !! 
...
!$ USE omp_lib           ! Note OpenMP sentinel
   ...
   NAMELIST/nam_rebuild/ filebase, ndomain, dims, nchunksize, l_verbose
...
!1. Read in the namelist 
   ...
   dims(:) = ""
   nchunksize = 0
   INQUIRE(FILE='nam_rebuild', EXIST=ldum)
   IF (ldum) THEN
      OPEN( UNIT=numnam, FILE='nam_rebuild', FORM='FORMATTED', STATUS='OLD' )
      READ( numnam, nam_rebuild )
      CLOSE( numnam )
   ELSE
      WRITE(numout,*) 'Namelist file nam_rebuild not found.'
      WRITE(numout,*) 'Reading namelist from standard input...'
      READ( *, nam_rebuild )
   END IF
   ! Document namelist
   IF (l_verbose) THEN
      WRITE(numout,*) 'nam_rebuild: '
      WRITE(numout,nam_rebuild)
   END IF
