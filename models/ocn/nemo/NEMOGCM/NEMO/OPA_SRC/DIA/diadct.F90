...
CONTAINS
  ...
  SUBROUTINE transport(sec,ld_debug)
     ...
     !! * Local variables
     INTEGER             :: jk,jseg,jclass,jl,   &!loop on level/segment/classes/ice type 
                            isgnu  , isgnv     !
        ...
        DO jseg=1,MAX(sec%nb_point-1,0)
           ...
           IF( sec%ll_ice_section )THEN
              ...
              DO jl=1,jpl
                 ...
                 IF( zTnorm .GE. 0)THEN
                    zice_vol_pos = zice_vol_pos + zTnorm*   &
                    ...
                    zice_surf_pos = zice_surf_pos + zTnorm * a_i(sec%listPoint(jseg)%I,sec%listPoint(jseg)%J,jl) 
                 ELSE

                    zice_vol_neg = zice_vol_neg + zTnorm*   &
                                      a_i(sec%listPoint(jseg)%I,sec%listPoint(jseg)%J,jl)     * &
                                     ( ht_i(sec%listPoint(jseg)%I,sec%listPoint(jseg)%J,jl) +   &
                                       ht_s(sec%listPoint(jseg)%I,sec%listPoint(jseg)%J,jl) )
                                 
                    zice_surf_neg = zice_surf_neg + zTnorm * a_i(sec%listPoint(jseg)%I,sec%listPoint(jseg)%J,jl)
                 ENDIF
