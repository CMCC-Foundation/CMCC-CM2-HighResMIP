MODULE diawri
   ...
   USE zdfkpp, ONLY: ghats  ! vertical  physics: KPP
#endif
...
# endif
   ...
   SUBROUTINE dia_wri_state( cdfile_name, kt )
      ...
      CALL histdef( id_i, "sometauy", "Meridional Wind Stress", "N/m2"   ,   &   ! j-wind stress
         &          jpi, jpj, nh_i, 1  , 1, 1  , -99 , 32, clop, zsto, zout )
      CALL histdef( id_i, "votkeavt", "vertical eddy diffusivity", "m2/s",   &   ! avt
         &          jpi, jpj, nh_i, jpk, 1, jpk, nz_i, 32, clop, zsto, zout )
      CALL histdef( id_i, "votkeavm", "vertical eddy viscosity", "m2/s",     &   ! avmu
         &          jpi, jpj, nh_i, jpk, 1, jpk, nz_i, 32, clop, zsto, zout )
      IF( lk_zdfddm ) THEN
        CALL histdef( id_i, "voddmavs", "salt vertical eddy diffusivity", "m2/s", &! avs
         &          jpi, jpj, nh_i, jpk, 1, jpk, nz_i, 32, clop, zsto, zout )
      END IF
#if defined key_zdfkpp
      CALL histdef( id_i, "voghats", "non-local scalar mixing term", "m2/s",   &   ! ghats
         &          jpi, jpj, nh_i, jpk, 1, jpk, nz_i, 32, clop, zsto, zout )
#endif
      ...
      END IF
#if defined key_zdfkpp
      CALL histwrite( id_i, "voghats", kt, ghats     , jpi*jpj*jpk, idex )    ! ghats
#endif
