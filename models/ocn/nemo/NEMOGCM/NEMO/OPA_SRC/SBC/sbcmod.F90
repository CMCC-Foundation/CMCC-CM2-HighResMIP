...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: sbcmod.F90 2715 2011-03-30 15:58:35Z rblod $
...
CONTAINS
   ...
   SUBROUTINE sbc_init
...
#if defined CCSMCOUPLED
         WRITE(numout,*) '          *** runoff from CESM cpl in rnf (NOT in emp)   ln_rnf_cpl  = ', ln_rnf_cpl
#else
      ...
      !                          ! Checks:
#if defined CCSMCOUPLED
      IF( .NOT. ln_rnf_cpl ) THEN                  ! no specific treatment in vicinity of river mouths 
#else
...
# if defined key_lim2 || defined key_lim3
      IF (lk_cesm) &
        CALL ctl_stop( 'sbc_init: key_lim2/3 defined within CESM coupled framework (key CCSMCOUPLED) !' )
# endif
#if defined CCSMCOUPLED
      IF (ln_rnf) THEN
        CALL ctl_stop( 'sbc_init: ln_rnf is incompatible with CESM coupled framework (key CCSMCOUPLED) !' )
      ENDIF
...
#if defined CCSMCOUPLED
      IF( ln_cpl          ) THEN   ;   nsbc =  8   ; icpt = icpt + 1   ;   ENDIF       ! NCAR CESM       formulation
#else
...
#if defined CCSMCOUPLED
         IF( nsbc ==  8 )   WRITE(numout,*) '          *** NCAR CESM coupled formulation ***'
#else
...
#if defined CCSMCOUPLED
      CASE(  8 )   ;   CALL sbc_cpl_cesm_rcv ( kt, nn_fsbc, nn_ice )   ! NCAR CESM formulation
#endif
      CASE( -1 )                                
...
#if defined CCSMCOUPLED
      IF( ln_rnf_cpl   )   CALL sbc_rnf( kt )                   ! handle runoff in CESM coupled case
#else
...
#if defined CCSMCOUPLED
      IF ( ln_rnf_cpl ) THEN
         CALL iom_put( "rnf_x_sst", -rnf*tsn(:,:,1,jp_tem) )   ! c/d term on sst
         CALL iom_put( "rnf_x_sss", -rnf*tsn(:,:,1,jp_sal) )   ! c/d term on sss
      ENDIF
#endif
      !
