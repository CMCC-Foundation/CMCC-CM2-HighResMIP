...
!! REAL(wp), PUBLIC ::   omega = 7.292115083046061e-5_wp ,  &  !: change the last digit!
   ...
   REAL(wp), PUBLIC ::   rcp      = SHR_CONST_CPSW  !: ocean specific heat
#else
...
#if defined CCSMCOUPLED
   REAL(wp), PUBLIC ::   rhoic   = SHR_CONST_RHOICE  !: volumic mass of sea ice (kg/m3)
#else
   ...
   REAL(wp), PUBLIC ::   cevap   = SHR_CONST_LATVAP       !: latent heat of evaporation (water)
#else
