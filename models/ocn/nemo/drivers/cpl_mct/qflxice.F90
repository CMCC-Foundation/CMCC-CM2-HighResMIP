...
!   SFLUX(:,:) = 0.0_wp
   ...
   if (ln_rstart) then
      ...
      call iom_get( numror, jpdom_autoglo, 'AQICE', AQICE(:,:) )
      call iom_get( numror, jpdom_autoglo, 'QFLUX', QFLUX(:,:) )
!!      IF (iom_varid( numror, 'SFLUX', ldstop = .FALSE. ) > 0) &
!        call iom_get( numror, jpdom_autoglo, 'SFLUX', SFLUX(:,:) )
!!      IF (iom_varid( numror, 'SALT_FREEZE', ldstop = .FALSE. ) > 0) &
!        call iom_get( numror, jpdom_autoglo, 'SALT_FREEZE', SALT_FREEZE(:,:) )
   endif
...
!
       ...
       POTICE(:,:) = (TFRZ(:,:) - tsa(:,:,k,jp_tem))*fse3t(:,:,k)*tmask(:,:,k)
!
!     !***
...
!***********************************************************************
   ...
   subroutine ice_flx_to_coupler( kt )
...
!-----------------------------------------------------------------------
   ...
   WORK2(:,:) = 0.0_wp
   WORK2(:,:) = (TFRZ(:,:) - tsn(:,:,1,jp_tem)) * WORK1(:,:) * tmask(:,:,1)
...
!     SFLUX(:,:) = 0.0_wp
!   else
!     SFLUX(:,:) = SALT_FREEZE(:,:)*rau0*WORK1(:,:)*tmask(:,:,1)/tlast_ice
!   endif
   ...
   if (tlast_ice == 0.0_wp) then
     ...
     QFLUX(:,:) = WORK1(:,:)*tmask(:,:,1)*rau0*rcp/tlast_ice
   endif
